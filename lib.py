...
class FireCode(object): # Класс для кода Файра
    ...
    def add_errors2(self, err1, err2, message):
        if err1 == 0 and err2 == 0:
            return message
        message = np.array(message)
        if message[err1-1] == 0:     # Изменяем по указанному месту 1го бита на противоположный
            message[err1-1] = 1
        else:
            message[err1-1] = 0
        if message[err2] == 0:     # Изменяем по указанному месту 2го бита на противоположный
            message[err2] = 1
        else:
            message[err2] = 0
        return message # Возвращаем испорченное сообщение
